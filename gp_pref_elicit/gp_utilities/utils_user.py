...
class UserPreference:
    ...
    def _initialise_utility_function(self):
        ...
        if self.num_objectives == 1:
            ...
            utl_func = lambda x: np.ones((x.shape[0],x.shape[1])) - np.polyval(self.poly, x)
            ...
            weight_preference_funcs = self.random_state.uniform(0.2, 0.8, len(funcs_1d)) #zdt1
            # weight_preference_funcs = np.array([.0,1.0,.0,.0,0.,0,.0,.0])
            # weight_preference_funcs = np.array([0.9,0.9,0])

            # weight_preference_funcs = np.array([0.8,0.2]) # zdt2
            # weight_preference_funcs = np.array([0.4,-0.15]) # zdt3
            # weight_preference_funcs = np.array([0.35,0.3])   # zdt4 
            # weight_preference_funcs = np.array([0.3,0.9])    # zdt6
            # weight_preference_funcs = np.array([0.1,0.1,1])   #dtlz2-3d
            ...
            def utl_func(x):
                ...
                for d in range(len(funcs_1d)):
                    y += weight_preference_funcs[d] * (1 - funcs_1d[d](x[:, d % self.num_objectives]))

                # return y
                # for minimization problems, the closer objectives are to 0, the bigger utility values are
                return y
...
def _mon_utilities_1d(random_state):
    ...
    for _ in range(1):
        ...
        polys.append(lambda x: _poly(x, b))
        polys.append(lambda x: _poly(x, c))
        polys.append(lambda x: _poly(x, d))
        polys.append(lambda x: _poly(x, e))
        polys.append(lambda x: _poly(x, f))
        polys.append(lambda x: _poly(x, g))
        polys.append(lambda x: _poly(x, h))
        polys.append(lambda x: _poly(x, i))
        polys.append(lambda x: _poly(x, j))
