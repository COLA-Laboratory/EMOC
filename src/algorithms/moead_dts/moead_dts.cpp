...
namespace emoc{
	...
	MOEADDTS::MOEADDTS(int thread_id) :
		Algorithm(thread_id),
		lambda_(nullptr),
		weight_num_(0),
		neighbour_(nullptr),
	...
	void MOEADDTS::Run()
	{
		...
		while (!IsTermination())
		{
			...
			{
				alpha_[i] = gamma_ * alpha_[i];
				beta_[i] = gamma_ * beta_[i];
			}
	...
	void MOEADDTS::UpdateBetaDis(int op, double reward)
	{
	...
	void MOEADDTS::Crossover(int operator_index, Individual** parent_pop, int current_index, Individual* offspring)
	{
	...
	double MOEADDTS::UpdateSubproblem(Individual* offspring, int current_index)
	{
	...
	void MOEADDTS::SelectCurrentSubproblem()
	{
	...
	void MOEADDTS::CalculateFitness(Individual** pop, int pop_num, double* fitness)
	{
