...
#define EMOC_REGIST_ALGORITHM(CATEGORY,NAME)\
Algorithm * CreateAlgorithm##NAME(int thread_id) {\
	return new NAME(thread_id);\
}\
AlgorithmRegisterAction NAME##Register(#CATEGORY, #NAME, CreateAlgorithm##NAME);

#define EMOC_REGIST_PROBLEM(CATEGORY,NAME)\
Problem * CreateProblem##NAME(int dec_num, int obj_num) {\
	return new NAME(dec_num, obj_num);\
}\
ProblemRegisterAction NAME##Register(#CATEGORY, #NAME, CreateProblem##NAME);